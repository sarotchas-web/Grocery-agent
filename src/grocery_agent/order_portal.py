...
def compare_order_form(form: dict[str, str]) -> OrderComparison:
    ...
    if not offers:
        raise ValueError("Add at least one retailer quote.")
    try:
        recommendation = recommend(offers)
    except ValueError as exc:
        raise ValueError("At least one retailer must have the full basket available for delivery.") from exc
...
def render_new_order_form(actor_id: str) -> str:
    ...
    <section class="toolbar">
      <a href="/?actor={escape(actor_id)}">Back</a>
    </section>
    <section class="band">
      <p class="eyebrow">New order</p>
      <h1>Build your shopping list</h1>
      <p class="muted">The list stays on this computer and is not sent to a retailer.</p>
    </section>
    <form method="post" action="/orders/quotes" class="form order-form">
      <input type="hidden" name="actor" value="{escape(actor_id)}">
      <label>Shopping items
        <textarea name="items" rows="10" required placeholder="Milk&#10;Bread&#10;Apples"></textarea>
      </label>
      <button type="submit">Continue to retailer quotes</button>
    </form>
    """
...
def render_quote_form(actor_id: str, items_raw: str) -> str:
    ...
    return f"""
    <section class="toolbar">
      <a href="/orders/new?actor={escape(actor_id)}">Back</a>
    </section>
    <section class="band">
      <p class="eyebrow">Retailer comparison</p>
      <h1>Add current basket quotes</h1>
      <p class="muted">Use current figures from each retailer. No retailer account is connected.</p>
    ...
    <form method="post" action="/orders/recommend" class="quote-form">
      ...
      {_retailer_fields("a", "Retailer 1")}
      {_retailer_fields("b", "Retailer 2", required=False)}
...
def render_comparison(actor_id: str, comparison: OrderComparison) -> str:
        ...
        for pickup in eligible_pickup_estimates(offer.delivery, offer.pickup_points, PickupPreference()):
            ...
            alternatives.append(
                ...
                <tr>
                  ...
                  <td>{escape(str(summary["pickup_window"]))}</td>
    ...
    acknowledgement = (
        ...
        else '<p class="ok">No budget acknowledgement required.</p>'
    )
    ...
    recommended_rows = "".join(
        f"<li><strong>{escape(order.retailer)}</strong> | {order.mode.value} | \u20aa{order.final_total}</li>"
        for order in recommendation.orders
    )
    return f"""
    <section class="toolbar">
      <a href="/orders/new?actor={escape(actor_id)}">Start over</a>
    </section>
    <section class="band recommendation">
      ...
      <p class="total">Estimated total <strong>\u20aa{total}</strong></p>
      {f'<p class="warning" lang="he" dir="rtl">{escape(weighted_notice)}</p>' if weighted_notice else ''}
    </section>
    <section class="table-band">
      <h2>Compared options</h2>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Retailer</th><th>Mode</th><th>Pickup point</th><th>Window</th><th>Pickup fee</th><th>Final total</th><th>vs delivery</th></tr></thead>
    ...
    <form method="post" action="/orders/approve" class="form approval-form">
      ...
      {acknowledgement}
      <button type="submit">Approve cart preparation</button>
    </form>
    """


def render_approval(form: dict[str, str]) -> str:
    total = _decimal(form.get("estimated_total", ""), "estimated total")
    if BudgetPolicy().requires_acknowledgement(total) and form.get("budget_ack") != "yes":
        raise PermissionError(BUDGET_ACK_TEXT_HE)
    return f"""
    <section class="toolbar"><a href="/">Home</a></section>
    <section class="band success">
      <p class="eyebrow">Cart preparation</p>
      <h1>Approved</h1>
      <p>Strategy: <strong>{escape(form.get("strategy", "selected option"))}</strong></p>
      <p class="total">Estimated total <strong>\u20aa{total}</strong></p>
      <p class="muted">No purchase, payment, or retailer checkout was performed.</p>
    </section>
    """
...
def _offer_from_form(prefix: str, form: dict[str, str]) -> RetailerOffer | None:
    ...
    subtotal = _decimal(form.get(f"{prefix}_subtotal", ""), f"{retailer} subtotal")
    basket = (
        BasketItem(
            sku=f"{prefix}-basket",
    ...
    if form.get(f"{prefix}_pickup") == "yes":
        ...
        if not point_name or not start or not end:
            raise ValueError(f"Complete the pickup details for {retailer}.")
        pickup_points = (
            PickupPoint(
...
def _retailer_fields(prefix: str, title: str, required: bool = True) -> str:
    ...
    <fieldset>
      ...
      <div class="fields">
        <label>Retailer <input name="{prefix}_retailer" {required_attr}></label>
        <label>Items subtotal (ILS) <input name="{prefix}_subtotal" type="number" min="0" step="0.01" {required_attr}></label>
        <label>Item discounts <input name="{prefix}_discounts" type="number" min="0" step="0.01" value="0"></label>
        <label>Promotions <input name="{prefix}_promotions" type="number" min="0" step="0.01" value="0"></label>
        <label>Delivery fee <input name="{prefix}_delivery_fee" type="number" min="0" step="0.01" value="0"></label>
        <label>Service fees <input name="{prefix}_service_fee" type="number" min="0" step="0.01" value="0"></label>
      </div>
      <div class="checks">
        <label><input type="checkbox" name="{prefix}_available" value="yes" checked> Full basket available</label>
        <label><input type="checkbox" name="{prefix}_weighted" value="yes"> Includes weighted products</label>
        <label><input type="checkbox" name="{prefix}_pickup" value="yes"> Pickup offered</label>
      </div>
      <div class="fields pickup-fields">
        <label>Pickup point <input name="{prefix}_pickup_name"></label>
        <label>Area <input name="{prefix}_pickup_area" placeholder="Emek Hefer"></label>
        <label>Window start <input name="{prefix}_pickup_start" type="time"></label>
        <label>Window end <input name="{prefix}_pickup_end" type="time"></label>
        <label>Pickup fee <input name="{prefix}_pickup_fee" type="number" min="0" step="0.01" value="0"></label>
      </div>
    </fieldset>
    """
...
def _strategy_title(strategy: str) -> str:
    return {
        "one_retailer_delivery": "One-retailer delivery",
        "one_retailer_pickup": "One-retailer pickup",
        "split_basket": "Split basket",
